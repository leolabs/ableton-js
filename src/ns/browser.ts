...
import { BrowserItem, RawBrowserItem } from "./browserItem";
...
> {
  ...
  constructor(ableton: Ableton, public raw: RawBrowser) {
    ...
    this.transformers = {
      ...
      audio_effects: (audio_effects) => audio_effects.map((audio_effect) => new BrowserItem(ableton, audio_effect)),
      clips: (clips) => clips.map((clip) => new BrowserItem(ableton, clip)),
      colors: (colors) => colors.map((color) => new BrowserItem(ableton, color)),
      current_project: (current_project) => current_project.map((c) => new BrowserItem(ableton, c)),
      drums: (drums) => drums.map((drum) => new BrowserItem(ableton, drum)),
      instruments: (instruments) => instruments.map((instrument) => new BrowserItem(ableton, instrument)),
      max_for_live: (max_for_live) => max_for_live.map((max) => new BrowserItem(ableton, max)),
      midi_effects: (midi_effects) => midi_effects.map((midi_effect) => new BrowserItem(ableton, midi_effect)),
      packs: (packs) => packs.map((pack) => new BrowserItem(ableton, pack)),
      plugins: (plugins) => plugins.map((plugin) => new BrowserItem(ableton, plugin)),
      samples: (samples) => samples.map((sample) => new BrowserItem(ableton, sample)),
      sounds: (sounds) => sounds.map((sound) => new BrowserItem(ableton, sound)),
      user_library: (user_library) => user_library.map((user) => new BrowserItem(ableton, user)),
      user_folders: (user_folders) => user_folders.map((user_folder) => new BrowserItem(ableton, user_folder)),
  ...
  public async load_item(item: BrowserItem) {
    return this.sendCommand("load_item", {id:item.raw.id});
  }
  public async preview_item(item: BrowserItem) {
    return this.sendCommand("preview_item", {id:item.raw.id});
  }
  public async stop_preview() {
